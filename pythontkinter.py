# run the program
# enter on the username as admin/rathore and password as 123456/1234
# if valid; click on the OK button
# if not; try to check database and then go to view table and check for databases feed
# after entering into db; perform basic tasks
# exit; manually
...
con = mysql.connector.connect(
    host="localhost", user="root", password="Amit@mysql1", database="dbmsproject")
check = 0
...
def enter():
    def addcar():
        ...
        x = 'y'
        cur.execute(
            "insert into cars(car_id,brand,rent,available) values(%s,%s,%s,%s)", (e1, e2, e3, x))
        con.commit()
        carid.delete(0, END)
        brand.delete(0, END)
        price.delete(0, END)
...
# function for deleting a car
    def delcar():
        ...
        x = 'y'
        cur.execute(
            "delete from cars where car_id=%s and available=%s", (e1, x))
        con.commit()
        carid.delete(0, END)
        brand.delete(0, END)
        price.delete(0, END)
...
# creating the main window
    ...
    root.title("DBMS PROJECT by Rathore")
    ...
    def bookcar():
        ...
        cur = con.cursor()
        cur.execute(
            "insert into users(name,email,startdate,loc) values(%s,%s,%s,%s)", (b, c, d, e))
        con.commit()
        cur = con.cursor()
        cur.execute(
            "select user_id from users where name=%s and email=%s", (b, c))
        ac = 0
        for r in cur:
            ac = r[0]
        con.commit()
        ...
        n = 'n'
        cur.execute(
            "insert into manages(user_id,car_id) values(%s,%s)", (ac, a))
        x = 'n'
        ...
        # con.commit()
        cur.execute(
            "delete from manages where car_id not in (select car_id from cars)")
        con.commit()
        cur.execute(
            "delete from users where user_id not in (select user_id from manages)")
        con.commit()
        messagebox.showinfo("success", "car booked")
    ...
    def returncar():
        ...
        cur = con.cursor()
        cur.execute(
            "select car_id,user_id,name,email from manages natural join users natural join cars where available='n'")
        i = 0
        for row in cur:
            tree1.insert('', i, text='', values=(
        ...
        def rccar():
            ...
            y = 'y'
            cur.execute(
                "update cars set available=%s where car_id=%s", (y, s1))
            con.commit()
            cur.execute(
                "delete from manages where car_id=%s and user_id=%s", (s1, s2))
            con.commit()
        ...
        b4 = Button(root1, text="RETURN CAR",
                    command=rccar).place(x=250, y=370)
    ...
    but4 = Button(frame3, text="GO TO RETURN CAR",
                  command=returncar).place(x=200, y=100)
